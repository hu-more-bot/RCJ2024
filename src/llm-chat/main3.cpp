...
void whisper_print_usage(int /*argc*/, char **argv,
  ...
  fprintf(
      stderr,
      "  -pms N,     --prompt-ms N    [%-7d] prompt duration in milliseconds\n",
      params.prompt_ms);
  fprintf(stderr,
  ...
  fprintf(
      stderr,
      "  -cmd FNAME, --commands FNAME [%-7s] text file with allowed commands\n",
      params.commands.c_str());
  fprintf(
      stderr,
      "  -p,         --prompt         [%-7s] the required activation prompt\n",
      params.prompt.c_str());
  fprintf(stderr,
          "  -ctx,       --context        [%-7s] sample text to help the "
          "transcription\n",
          params.context.c_str());
  fprintf(
      stderr,
      "  --grammar GRAMMAR            [%-7s] GBNF grammar to guide decoding\n",
      params.grammar.c_str());
  fprintf(stderr,
          "  --grammar-penalty N          [%-7.1f] scales down logits of "
          "nongrammar tokens\n",
  ...
  while (is_running) {
...

    {
      ...
      if (::vad_simple(pcmf32_cur, WHISPER_SAMPLE_RATE, 1000, params.vad_thold,
        ...
        //                           logprob_min0, logprob_sum0, n_tokens0,
        //                           t_ms));
        ...
        //     fprintf(stdout, "%s: WARNING: prompt not recognized, try
        //     again\n",
        //             __func__);
        //     ask_prompt = true;
        //   } else {
        ...
        // } else
        {
          ...
          pcmf32_cur.insert(pcmf32_cur.begin(), 3.0f * WHISPER_SAMPLE_RATE,
                            0.0f);
