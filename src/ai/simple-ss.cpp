...
struct whisper_params {
  ...
  int32_t max_len = 0;
  int32_t best_of =
      whisper_full_default_params(WHISPER_SAMPLING_GREEDY).greedy.best_of;
  int32_t beam_size = whisper_full_default_params(WHISPER_SAMPLING_BEAM_SEARCH)
...
bool whisper_params_parse(int argc, char **argv, whisper_params &params) {
  for (int i = 1; i < argc; i++) {
    ...
    } else if (arg == "-ac" || arg == "--audio-context") {
...
bool output_csv(struct whisper_context *ctx, const char *fname,
  ...
  for (int i = 0; i < n_segments; ++i) {
    ...
    // need to multiply times returned from whisper_full_get_segment_t{0,1}() by
...
bool output_json(struct whisper_context *ctx, const char *fname,
                 const whisper_params &params,
                 std::vector<std::vector<float>> pcmf32s, bool full) {
  std::ofstream fout(fname);
  int indent = 0;
...
bool output_wts(struct whisper_context *ctx, const char *fname,
  ...
  if (!fin.is_open()) {
    fprintf(stderr,
            "%s: font not found at '%s', please specify a monospace font with "
            "-fp\n",
            __func__, font);
    return false;
  }
  ...
  for (int i = 0; i < whisper_full_n_segments(ctx); i++) {
    ...
    for (int j = 0; j < n; ++j) {
      ...
      if (is_first) {
        // background text
        fout << ",drawtext=fontfile='" << font
             << "':fontsize=24:fontcolor=gray:x=(w-text_w)/2:y=h/2:text='"
      ...
      // foreground text
      fout << ",drawtext=fontfile='" << font
           << "':fontsize=24:fontcolor=lightgreen:x=(w-text_w)/2+8:y=h/2:text='"
...
int main(int argc, char **argv) {
  ...
  cparams.use_gpu = false; // TODO test

  const char *model = "../models/asd";
  ...
  for (int f = 0; f < (int)params.fname_inp.size(); ++f) {
    ...
    // run the inference
    {
      ...
      // the callback is called before every computation - if it returns true,
    ...
    // output stuff
    {
      ...
      if (params.output_wts) {
        ...
        output_wts(ctx, fname_wts.c_str(), fname_inp.c_str(), params,
                   float(pcmf32.size() + 1000) / WHISPER_SAMPLE_RATE, pcmf32s);
      }
